-- Le lieu d'intervention cesse de se perdre en devenant une facture.
--
-- Sur un bon de commande, `adresse` est l'adresse du CHANTIER : le formulaire
-- l'annonce ainsi, « Adresse d'intervention ». Sur une facture, `adresse`
-- designe l'adresse du CLIENT — le formulaire l'ecrase systematiquement avec
-- celle de sa fiche, et `adresse_locataire` porte le lieu des travaux.
--
-- `bc_generer_facture` recopiait pourtant l'une dans l'autre. Le lieu des
-- travaux atterrissait donc dans la case du siege du client, et le bloc
-- « Lieu d'intervention » du document imprime restait vide faute de trouver
-- `adresse_locataire`.
--
-- Ce que la production montre : sur 829 bons de commande, 5 renseignent
-- `adresse_locataire` et 83 mettent le chantier dans `adresse`. Sur 394
-- factures, 4 portent un lieu d'intervention. Le champ n'etait pas oublie par
-- les utilisateurs — il etait perdu a la transformation.
--
-- La fonction est remplacee en entier, comme toute fonction Postgres ; seule
-- la mise en correspondance des adresses change.
--
-- Les factures deja emises ne sont PAS corrigees ici : leur reprise est un
-- geste distinct, qui demande un accord explicite.
