-- La facture née d'un bon porte son échéance, comme celle née de l'écran.
--
-- Elle insérait `conditions_reglement = '30 jours'` en dur et ne renseignait
-- pas `echeance` du tout : le couple était contradictoire, et la date
-- manquait. Le délai vient du client, à défaut des réglages de la société,
-- lus là où ils se saisissent — `societe_settings.infos_entreprise`.
--
-- `COALESCE` et non un test de vérité : un client paie parfois à réception,
-- donc zéro jour, et zéro doit gagner contre le défaut.
