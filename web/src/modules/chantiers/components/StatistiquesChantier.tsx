        ...
        <dl className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8" aria-label="Chiffres du chantier">
          {s.avancement && (
            <div className="col-span-2 flex items-center gap-3">
              ...
              <Case valeur={formatEuros(s.avancement.total)} libelle="Total DPGF (HT) — avancement facturé" />
            </div>
          )}
          ...
          {s.totalAchats && <Case valeur={formatEuros(s.totalAchats)} libelle="Achats" />}
          {s.nbFactures !== null && <Case valeur={s.nbFactures} libelle={s.nbFactures > 1 ? "Factures" : "Facture"} />}
          {s.margeConstatee && <Case valeur={formatEuros(s.margeConstatee)} libelle="Facturé − achats" />}
