...
- **Écran** (`EcranLecture`, port d'`ocrEcranHTML`, D-ECR-BC-10) : dans le
  formulaire du bon (`commandes/hooks/useLectureDuBon`) — étapes (préparation,
  envoi, lecture par le modèle, bascule à 2 s), chronomètre, durée annoncée,
  « Annuler la lecture » ; trois issues (annulée, délai, échec) avec
  « ↻ Réessayer » et « Saisir à la main ». `PageLectureBon` ne fait plus que
  rediriger vers `/commandes/nouveau`.
...
- **Préremplissage** : le bouton d'import passe le fichier choisi
  (`navigate("/commandes/nouveau", { state: { lire: fichier } })`), lu sans être
  redemandé ; le résultat préremplit le formulaire — tout ce que `versSaisieBonCommande` transmettait (« Sans BC », date du jour,
