/**
 * La couleur d'une société, déclinée en palette (SOC-04).
 *
 * Portage TEL QUEL de `src/api/regles-theme.ts` (parité : tests/parite/theme.essai.ts).
 * Hors de `domain/` à dessein : les arrondis ici portent sur des canaux de
 * couleur 0-255, pas sur de l'argent — le garde-fou « pas de Math.round dans le
 * domaine » vise les montants (D-SOC-03).
 */
...
/**
 * Pour une couleur que ce module a FABRIQUÉE (ou une constante) : elle est bien
 * formée par construction. L'original écrivait `versRvb(x)!` ; une exception
 * dit mieux qu'un `undefined` silencieux ce qui aurait cassé.
 */
function versRvbSur(hex: string): [number, number, number] {
  const rvb = versRvb(hex);
  if (!rvb) throw new Error(`Couleur calculée mal formée : ${hex}`);
  return rvb;
}
...
function tonFonceLisible(h: number, s: number): string {
  for (let l = L_FONCE; l >= 0.2; l -= 0.01) {
    ...
    const rvb = versRvbSur(hex);
    if (contraste(rvb, versRvbSur(texteLisibleSur(rvb))) >= CONTRASTE_AA) return hex;
  }
...
export function paletteAccent(couleur?: string | null): PaletteAccent {
  ...
  return {
    ...
    surAccentFonce: texteLisibleSur(versRvbSur(accentFonce)),
  };
}
...
export function paletteSecondaire(couleur?: string | null): PaletteSecondaire {
  const rvb = versRvb(String(couleur ?? "")) ?? versRvbSur(SECONDAIRE_DEFAUT);
