...
async function parPages<T>(lire: (debut: number, fin: number) => PromiseLike<{ data: unknown; error: unknown }>, schema: z.ZodType<T>, contexte: string) {
  const tout: T[] = [];
...
function lots(ids: readonly string[]): string[][] {
  const sortie: string[][] = [];
...
async function tachesDes(ids: readonly string[], client: Client): Promise<TacheBon[]> {
  const pages = await Promise.all(
    lots(ids).map((lot) =>
      parPages((d, f) => client.from("planning_taches").select(COLONNES_TACHE).in("bon_commande_id", lot).order("cree_le").range(d, f), schemaTacheBon, "tâches des bons")
    )
  );
  return pages.flat();
}
...
async function facturesDes(ids: readonly string[], client: Client): Promise<FactureLiee[]> {
  const pages = await Promise.all(
    lots(ids).map((lot) => parPages((d, f) => client.from("factures").select("id, numero, bon_commande_id").in("bon_commande_id", lot).range(d, f), schemaFactureLiee, "factures des bons"))
  );
  return pages.flat();
}
...
export class EnregistrementPartiel extends Error {
  constructor(
    ...
    super("Le bon est enregistré, mais pas toutes ses lignes. Vérifiez-les puis enregistrez à nouveau.");
