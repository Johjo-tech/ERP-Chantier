...
describe("annuaire des entreprises dans la fiche client (CLI-02, CLI-23, CLI-40)", () => {
  it("le nom propose les entreprises ; le choix remplit l'identité et l'adresse, la TVA saisie reste", async () => {
    ...
    await userEvent.type(screen.getByLabelText("Nom / raison sociale"), "peinture martin");
    const liste = await screen.findByRole("group", { name: "Entreprises de l'annuaire" });
    ...
    expect(screen.getByLabelText("Nom / raison sociale")).toHaveValue("PEINTURE MARTIN");
    expect(screen.getByLabelText("SIRET / SIREN")).toHaveValue("73282932000074");
    ...
    expect(screen.getAllByLabelText("Code postal")[0] as HTMLElement).toHaveValue("75002");
    expect(screen.getAllByLabelText("Ville")[0] as HTMLElement).toHaveValue("PARIS");
  ...
  it("un acheteur public : le type « administration » est proposé, jamais imposé", async () => {
    await ouvrir();
    await userEvent.type(screen.getByLabelText("Nom / raison sociale"), "mairie");
    await userEvent.click(await within(await screen.findByRole("group", { name: "Entreprises de l'annuaire" })).findByRole("button", { name: /MAIRIE DE LYON/ }));
  ...
  it("un particulier n'interroge pas l'annuaire (« laurent johan » ramenait cinq SIRET)", async () => {
    ...
    await userEvent.type(screen.getByLabelText("Nom / raison sociale"), "laurent johan");
    await new Promise((r) => setTimeout(r, 500));
    expect(annuaire.rechercherEntreprise).not.toHaveBeenCalled();
    expect(screen.queryByRole("group", { name: "Entreprises de l'annuaire" })).not.toBeInTheDocument();
  });
  ...
  it("SIREN : les établissements ouverts au choix ; une entreprise radiée avertit sans bloquer", async () => {
    ...
    await ouvrir();
    // Comme l'ancien : le SIREN se cherche depuis le champ « SIRET / SIREN ».
    await userEvent.type(await screen.findByLabelText("SIRET / SIREN"), "732829320");
    await userEvent.click(screen.getByRole("button", { name: "🔍 Rechercher" }));
    expect(await screen.findByText("2 établissements ouverts — choisissez :")).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: /73282932000082/ }));
    expect(screen.getByLabelText("SIRET / SIREN")).toHaveValue("73282932000082");
  ...
  it("l'adresse propose la Base Adresse Nationale, qui remplit code postal et ville", async () => {
    ...
    await userEvent.click(await within(await screen.findByRole("group", { name: "Adresses proposées" })).findByRole("button", { name: /8 Boulevard du Port/ }));
    expect(screen.getByLabelText("Adresse")).toHaveValue("8 Boulevard du Port");
    expect(screen.getAllByLabelText("Code postal")[0] as HTMLElement).toHaveValue("80000");
    expect(screen.getAllByLabelText("Ville")[0] as HTMLElement).toHaveValue("Amiens");
  });
});
